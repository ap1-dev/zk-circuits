...
function textToFieldChunks(text, maxChunks) {
  ...
  if (chunks.length > maxChunks) {
    throw new Error(
      `Test log too large. Got ${chunks.length} chunks, max is ${maxChunks}. Increase MAX_LOG_CHUNKS in the circuit.`
    );
  }
...
*/
function poseidonChainedTestLogHash(poseidon, F, chunks, exit1, exit2, exit3) {
  if (chunks.length < 2) {
...
async function main() {
  ...
  /*
    ...
    In real pipeline:
      - TEE test script emits this log.
      - TEE extracts the structured exit codes.
      - TEE computes test_log_hash.
      - TEE puts test_log_hash into report_data.
  */
  const rawTestLog = `
random test output...
[STAGE 1]: Completed
...
`;
  ...
  const testLogChunks = textToFieldChunks(rawTestLog, MAX_LOG_CHUNKS);

  const stage1 = "0";
  const stage2 = "0";
  const stage3 = "0";

  const testLogHash = poseidonChainedTestLogHash(
    poseidon,
    F,
    testLogChunks,
    stage1,
  ...
  console.log("Public test_log_hash:", testLogHash);
}
