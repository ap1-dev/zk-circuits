...
/*
  ...
  Private:
    test_log_chunks[MAX_LOG_CHUNKS]
    stage_1_exit_code
    stage_2_exit_code
    stage_3_exit_code
  ...
  Claim:
    Poseidon-chained private test log == public test_log_hash
    stage_1_exit_code == 0
    stage_2_exit_code == 0
    stage_3_exit_code == 0
*/

template FTest(MAX_LOG_CHUNKS) {
    signal input test_log_chunks[MAX_LOG_CHUNKS];

    signal input stage_1_exit_code;
    signal input stage_2_exit_code;
    signal input stage_3_exit_code;

    signal input test_log_hash;
    ...
    */
    ...
    chunk_hashes[0].inputs[0] <== test_log_chunks[0];
    chunk_hashes[0].inputs[1] <== test_log_chunks[1];
    ...
    for (var i = 1; i < MAX_LOG_CHUNKS - 1; i++) {
        ...
        chunk_hashes[i].inputs[1] <== test_log_chunks[i + 1];
    }
    ...
    // Public hash check
    test_log_hash === final_hasher.out;
...
component main { public [test_log_hash] } = FTest(16);
